...
var MAC = {
    ...
    'History': {
        ...
        'Create': function($id) {
            ...
            html = '<section id="ls_box" class="searchMenu" style="display:none;"><div class="hisBox fn-left"><div class="looked-box" id="history"><div class="looked-list"><p><a class="close-his" href="javascript:void(0);" onclick="ls_box_hide();" target="_self">关闭</a><a class="clear-his" href="javascript:void(0);" onclick="ls_box_del()" target="_self">清空记录</a></p><ul class="highlight" id="playhistory">';

            if (jsondata.length > 0) {
                for ($i = 0; $i < jsondata.length; $i++) {
                    if ($i % 2 == 1) {
                        html += '<li class="no-his">';
                    } else {
                        html += '<li class="no-his">';
                    }
                    html += '<p><a href="' + jsondata[$i].link + '" class="hx_title">' + jsondata[$i].name + '</a></p>';
                }
            } else {
                html += '<li class="no-his"><p>暂无播放历史列表...</p>';
            }
            html += '</li>';
        ...
        'Insert': function(name, link, typename, typelink, pic) {
            ...
            if (jsondata != undefined) {
                ...
                jsonstr = '{video:[{"name":"' + name + '","link":"' + link + '","typename":"' + typename + '","typelink":"' + typelink + '"},';
                for ($i = 0; $i <= this.Limit; $i++) {
                    if (this.Json[$i]) {
                        jsonstr += '{"name":"' + this.Json[$i].name + '","link":"' + this.Json[$i].link + '","typename":"' + this.Json[$i].typename + '","typelink":"' + this.Json[$i].typelink + '"},';
                    } else {
                        break;
                    }
                }
                ...
                jsonstr = '{video:[{"name":"1111111111","link":"' + link + '","typename":"' + typename + '","typelink":"' + typelink + '"}]}';
            }
...
function ls_box_hide() {
    $('#ls_box').hide();
    $('.aUnLogin').removeClass("cur");
...
function ls_box_del() {
    ...
    MAC.Cookie.Del('mac_history');
    $('#playhistory').html('<li class="no-his"><p>暂无播放历史列表...</p>');
}